...
describe("post /api/blogs", () => {
    ...
    test("adding blog increases amount of blogs by one", async () => {
        const newBlog = {
            title: "Test Blog 4",
            author: "Different Test Person",
    ...
    test("correctly formatted blog is added", async () => {
        const newBlog = {
            title: "Test Blog 4",
            author: "Different Test Person",
            url: "www.testblog4.com",
    ...
    test("adding blog without title or author fails, denies with reason", async () => {
        const newBlog = {
            url: "www.testblog4.com",
            likes: 4
        }
        await api.post("/api/blogs").send(newBlog).expect(400, {error: "Blog validation failed: title: Title is required., author: Author is required."})
    })

    test("adding blog without likes will default likes to 0", async () => {
        const newBlog = {
            title: "Test Blog 4",
            author: "Different Test Person",
            url: "www.testblog4.com"
...
describe("delete /api/blogs/:id", () => {
    ...
    test("deleting note works", async () => {
        ...
        await api.delete(`/api/blogs/${beginningBlogs.body[0].id}`).expect(204)
...
describe("put /api/blogs/id", () => {
    ...
    test("updating with correct id works", async () => {
        ...
        const newBlog = await api.put(`/api/blogs/${idToUpdate}`).send(updatedBlog).expect(200)
