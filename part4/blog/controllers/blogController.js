...
blogRouter.delete("/:id", async (req, res) => {
    ...
    // we try to find the blog that should be deleted and find the associated user for it.
    // the list of the users blogs is then filtered, to remove the blog from the list
    //we save the user after deleting the block. if the blog deletion throws an error, the new blog-list for the user is never saved
    ...
    if (oldBlog.user.toString() !== req.token.id) {
        return res.status(401).json({error: "User requesting the deletion and creator of blog post are not the same person."})
    }
...
blogRouter.put("/:id", async (req, res) => {
    if (!req.token) {
        return res.status(401).json({error: "You need to be authorized to change blog information."})
    }
    const oldBlog = await Blog.findById(req.params.id)
    if (oldBlog === null) {
        return res.status(400).json({error: "Could not find requested ID."})
